...
|---|---|---|
| **PyPI token + first upload** ✅ | Strangers expect `pip install failpack`. | **Done in 1.5.5:** first upload live on [PyPI](https://pypi.org/project/failpack/). Lead docs with `pip install failpack` + `failpack demo --fast` (+ both hermetic demos); git install is fallback only. Do not re-upload from agents without a human-held token. |
...
| **Hermetic Cursor one-shot** ✅ | Same proof for `capture --cursor-latest` without a live Cursor install. | **Done in 1.5.9:** `failpack demo --cursor-hermetic` + bundled `cursor-agent-failure.jsonl`. Thin alias: [`examples/cursor-latest-hermetic.sh`](../examples/cursor-latest-hermetic.sh). Doctor / capture tip the CLI path first when no Cursor sessions. |
...
| **One unpaid stranger walkthrough** | Someone who did not write FailPack should finish install → `demo --fast` → optional hermetic demos / real capture without a Slack babysitter. | Use `examples/STRANGER_WALKTHROUGH.md`; capture friction notes, fix bugs only. Agent may reduce friction (hermetic CLI); unpaid human still open. |
...
`demo --fast` / hermetic demos may leave packs under `.failpack/packs/` in a
repo checkout. That is expected for the stranger wow path; tests must tolerate
the extra goldens (do not hardcode an exact pack inventory of 4). Optional
cleanup: `failpack rm demo-five-minute --force` /
