...
Status as of **1.5.7**: product loop is real (`demo --fast` → capture →
promote → replay → CI Action). **Hermetic Claude one-shot**
(`capture --claude-latest` via fake HOME + fixture) is first-class, tested,
and tipped from doctor / CLI when no sessions exist. Support path is stated.
**PyPI is live** (`pip install failpack` since 1.5.5). Static homepage + Pages
...
| **Hermetic Claude one-shot** ✅ | Stranger/CI must prove `capture --claude-latest` without a live agent or invented “user” packs. | **Done in 1.5.7:** pytest e2e (fake HOME + `fixtures/claude-code-failure.jsonl` → promote → replay) + [`examples/claude-latest-hermetic.sh`](../examples/claude-latest-hermetic.sh). Doctor / capture errors tip fixture path / `demo --fast` first when no sessions. |
...
| **One unpaid stranger walkthrough** | Someone who did not write FailPack should finish install → `demo --fast` → optional `capture --claude-latest` without a Slack babysitter. | Use `examples/STRANGER_WALKTHROUGH.md`; capture friction notes, fix bugs only. Agent may reduce friction (hermetic tip path); unpaid human still open. |
...
failpack lint
failpack replay --all
./examples/claude-latest-hermetic.sh   # hermetic Claude one-shot (checkout)
pytest -q
```
